...
/* 
  ...
  2. Check if there's a `slug` in the query params:
    a. If there is no slug, it means that it's the generic internal-short.shopmy.com.au links
      i. Make sure the domain is `internal-short.shopmy.com.au` (prevent query injection)
...
const withLinksAuth =
  ...
  async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    // if there is no slug, it's the default internal-short.shopmy.com.au link
    if (!slug) {
      // prevent domain from being query injected by
      // making sure that all instances of `domain` are `internal-short.shopmy.com.au`
      if (
        (domain && domain !== "internal-short.shopmy.com.au") ||
        (req.body.domain && req.body.domain !== "internal-short.shopmy.com.au")
      ) {
    ...
    // if key is defined, check if the  current user is the owner of the link (only for internal-short.shopmy.com.au links)
    const { key } = req.query;
    if (key && !domain && !skipKeyCheck) {
      ...
      if (!link) {
        ...
        // for internal-short.shopmy.com.au links, check if the user is the owner of the link
